...
f=fopen('C:\\temp\\genlte.uplink.bin', 'rb'); fseek(f, 0.0*(7.68e6 * (2*2)), 'bof');v=fread(f, 10*7.68e6*(2), 'int16'); cv_usrp=v(1:2:end)+v(2:2:end)*j; fclose(f);
...
for i=0:length(chirp1)-1,
    remain = min(length(chirp_ref762), length(chirp1)-i);
    corrold(i+1) = sum((chirp1(1+i:remain+i) .* conj(chirp_ref762(1:remain))));
end
