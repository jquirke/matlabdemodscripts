...
f=fopen('c:\\temp\\genlte.uplink.123_3_14_5.bin', 'rb'); fseek(f, 0.0*(7.68e6 * (2*2)), 'bof');v=fread(f, 10*7.68e6*(2), 'int16'); cv_usrp=v(1:2:end)+v(2:2:end)*j; fclose(f);
...
chirp_ref = gen_prach_seq_base(123,14,0,25,5,4);
corrzoom = conv(cv_usrp, fliplr(conj(chirp_ref)));
chirp_ref = gen_prach_seq_base(123,14,1,25,5,4);
corrzoom2 = conv(cv_usrp, fliplr(conj(chirp_ref)));
chirp_ref = gen_prach_seq_base(123,14,2,25,5,4);
corrzoom3 = conv(cv_usrp, fliplr(conj(chirp_ref)));
chirp_ref = gen_prach_seq_base(124,14,0,25,5,4);
