%generate the 839 element sequence for a particular prach root, as well as
%the cyclic shift if any for a particular Zero Correlation Zone Config
%(zczc)

%currently only support premable format 0-3
%and unrestricted set (no high speed mode)

function x_u_v = gen_prach_seq(root_seq, zczc, v)

    %5.7.2-2
    N_CS_tab = [0 13 15 18 22 26 32 38 46 59 76 93 119 167 279 419]
    
    %5.7.2-4
    u_tab = [129, 710, 140, 699, 120, 719, 210, 629, 168, 671, 84, 755, 105, 734, 93, 746, 70, 769, 60, 779, 2, 837, 1, 838, ...
        ...
        u = u_tab(root_seq+1);
        NZC = 839; %for format 0-3
        NCS = N_CS_tab(zczc+1)
        
        n = [0:NZC-1];
        %base sequence
        x_u = exp(-j*pi*u.*n.*(n+1)/NZC);
        
        %testing circshift([1:10]', -2)'
        %sanity check
        if (NCS ~= 0)
            numshifts = floor(NZC/NCS); %not needed
            if (v >= numshifts)
                error('v must be lower than floor(NZC/NCS)');
            end
        else
            if (v ~= 0)
                error ('no shifts allowed for ZCZC=0');
            end
        end
        %also works for NCS = 0
        x_u_v = circshift(x_u', -v*NCS)';
        %else %1 entire root per sequence
        %    x_u_v = x_u;
        %end
end
