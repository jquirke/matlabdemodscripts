...
f=fopen('c:\\temp\\genlte.uplink.bin', 'rb'); fseek(f, 0.9*(7.68e6 * (2*2)), 'bof');v=fread(f, 1.1*7.68e6*(2), 'int16'); cv_usrp=v(1:2:end)+v(2:2:end)*j; fclose(f);
...
chirp_ref = gen_prach_seq_base(123,14,0,25,8,4);
corrzoom = conv(cv_usrp, fliplr(conj(chirp_ref)));
chirp_ref = gen_prach_seq_base(123,14,1,25,8,4);
corrzoom2 = conv(cv_usrp, fliplr(conj(chirp_ref)));
chirp_ref = gen_prach_seq_base(123,14,2,25,8,4);
corrzoom3 = conv(cv_usrp, fliplr(conj(chirp_ref)));
chirp_ref = gen_prach_seq_base(124,14,0,25,8,4);
corrzoom4 = conv(cv_usrp, fliplr(conj(chirp_ref)));


plot([0:length(cv_usrp)-1], abs(cv_usrp), [0:length(cv_usrp)-1], abs(corrzoom(1:length(cv_usrp))), [0:length(cv_usrp)-1], abs(corrzoom2(1:length(cv_usrp))), [0:length(cv_usrp)-1], abs(corrzoom3(1:length(cv_usrp))), [0:length(cv_usrp)-1], abs(corrzoom4(1:length(cv_usrp))))
...
figure(2); prachcorr = abs(ifft(fft(gen_prach_seq(124, 0, 0)).*conj(prach_extract_freq(0,25,10,chirp1,512)))).*2; max(prachcorr)/mean(prachcorr), plot(prachcorr);
