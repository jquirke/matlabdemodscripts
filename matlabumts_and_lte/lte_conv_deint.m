function deintbits = lte_conv_deint( intbits)
%Very simple linear estimation
%   Detailed explanation goes here

    CsubblockCC = 32;
    RsubblockCC = ceil(length(intbits)/CsubblockCC);
    ND = CsubblockCC * RsubblockCC - length(intbits);
    
    matrix = zeros(1,CsubblockCC * RsubblockCC);
    matrix(1:ND) = 0;
    matrix(ND+1:end) = [1:length(intbits)];
